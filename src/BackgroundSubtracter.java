...
public class BackgroundSubtracter {
	...
	private static boolean staticCreateBackground; // don't subtract background (e.g., for processing the background
													// before subtracting)
	private static boolean staticUseParaboloid; // use "Sliding Paraboloid" instead of rolling ball algorithm
	private static boolean staticDoPresmooth = false; // smoothen the image before creating the background
	...
	private final static int X_DIRECTION = 0, Y_DIRECTION = 1, DIAGONAL_1A = 2, DIAGONAL_1B = 3, DIAGONAL_2A = 4,
	...
	public void rollingBallBrightnessBackground(ColorProcessor ip, double radius, boolean createBackground,
		...
		rollingBallBackground(bp, radius, createBackground, lightBackground, useParaboloid, doPresmooth,
				correctCorners);
		ip.setHSB(H, S, (byte[]) bp.getPixels());
	}

	public double rollingBallBackground(ImageProcessor ip, double radius, boolean createBackground,
			boolean lightBackground, boolean useParaboloid, boolean doPresmooth, boolean correctCorners) {
			...
			if (createBackground)
				...
				if (ip instanceof FloatProcessor) { // here ip and fp are the same (bgPixels will be output)
					...
					for (int p = 0; p < bgPixels.length; p++) {
						...
						if (value > 65535f)
							value = 65535f;
						if (valueToSubtract > 65535f)
							valueToSubtract = 65535f;

						pixels[p] = (short) (value);
						pixelsToSubtract[p] = (short) (valueToSubtract);

					}
					double total = 0;
					for (int i = 0; i < pixelsToSubtract.length; i++)
						total = total + pixelsToSubtract[i];

					meanValue = total / pixelsToSubtract.length;
					...
					for (int p = 0; p < bgPixels.length; p++) {
						...
						if (value > 255f)
							value = 255f;
						if (valueToSubtract > 255f)
							valueToSubtract = 255f;
						pixels[p] = (byte) (value);
						pixelsToSubtract[p] = (byte) (valueToSubtract);
	...
	static float[] lineSlideParabola(float[] pixels, int start, int inc, int length, float coeff2, float[] cache,
			int[] nextPoint, float[] correctedEdges) {
		...
		/*
		 * copy data to cache, determine the minimum, and find points with local
		 * curvature such that the parabola can touch them - only these need to be
		 * examined futher on
		 */
		...
		while (i1 < length - 1) {
			...
			for (int j = nextPoint[i1]; j < searchTo; j = nextPoint[j], recalculateLimitNow++) {
				...
				if (recalculateLimitNow == 0) { // time-consuming recalculation of search limit: wait a bit after slope
												// is updated
					double b = 0.5f * minSlope / coeff2;
					int maxSearch = i1 + (int) (b + Math.sqrt(b * b + (v1 - minValue) / coeff2) + 1); // (numeric
																										// overflow may
																										// make this
																										// negative)
		...
		/*
		 * Now calculate estimated edge values without an edge particle, allowing for
		 * vignetting described as a 6th-order polynomial:
		 */
		if (correctedEdges != null) {
			...
			float slope = (v2 - v1) / (lastCorner - firstCorner); // of the line through the two outermost non-edge
																	// touching points
			...
			for (int i = (length + 2) / 3; i <= (2 * length) / 3; i++) {// compare with mid-image pixels to detect
																		// vignetting
				float dx = (i - mid) * 2f / (lastCorner - firstCorner);
				float poly6 = dx * dx * dx * dx * dx * dx - 1f; // the 6th order polynomial, zero at firstCorner and
																// lastCorner
			...
			correctedEdges[0] = value0 + coeff6 * (dx * dx * dx * dx * dx * dx - 1f)
					+ coeff2 * firstCorner * firstCorner;
			dx = (lastCorner - mid) * 2f / (lastCorner - firstCorner);
			correctedEdges[1] = value0 + (length - 1) * slope + coeff6 * (dx * dx * dx * dx * dx * dx - 1f)
	 ...
	 * Analyzing the directions parallel to the edges and the diagonals, we average
	 * over the 3 correction values (found for the 3 directions)
	 */
	void correctCorners(FloatProcessor fp, float coeff2, float[] cache, int[] nextPoint) {
		...
		correctedEdges = lineSlideParabola(pixels, (height - 1) * width, 1, width, coeff2, cache, nextPoint,
				correctedEdges);
		...
		correctedEdges = lineSlideParabola(pixels, 0, 1 + width, diagLength, coeff2diag, cache, nextPoint,
				correctedEdges);
		corners[0] += correctedEdges[0];
		correctedEdges = lineSlideParabola(pixels, width - 1, -1 + width, diagLength, coeff2diag, cache, nextPoint,
				correctedEdges);
		corners[1] += correctedEdges[0];
		correctedEdges = lineSlideParabola(pixels, (height - 1) * width, 1 - width, diagLength, coeff2diag, cache,
				nextPoint, correctedEdges);
		corners[2] += correctedEdges[0];
		correctedEdges = lineSlideParabola(pixels, width * height - 1, -1 - width, diagLength, coeff2diag, cache,
				nextPoint, correctedEdges);
	 ...
	 */
	void rollingBallFloatBackground(FloatProcessor fp, float radius, boolean invert, boolean doPresmooth,
			RollingBall ball) {
	 ...
	 * defined by the intensity value at every point in the image. The center of the
	 * filtering object, a patch from the top of a sphere having radius BallRadius,
	 * is moved along each scan line of the image so that the patch is tangent to
	 * the image at one or more points with every other point on the patch below the
	 * corresponding (x,y) point of the image. Any point either on or below the
	 * patch during this process is considered part of the background. Shrinking the
	 * image before running this procedure is advised for large ball radii because
	 * the processing time increases with ball radius^2.
	 */
		...
		for (int y = -radius; y < height + radius; y++) { // for all positions of the ball center:
			...
			for (int x = -radius; x < width + radius; x++) {
				...
				for (int yp = y0, yBall = yBall0; yp <= yend; yp++, yBall++) // raise pixels to ball surface
					for (int xp = x0, p = xp + yp * width, bp = xBall0
							+ yBall * ballWidth; xp <= xend; xp++, p++, bp++) {
						float zMin = z + zBall[bp];
						if (pixels[p] < zMin)
							pixels[p] = zMin;
					}
				// if (x>=0&&y>=0&&x<width&&y<height) bgPixels[x+y*width] = z; //debug, ball
				// height output
			}
		}
	...
	void enlargeImage(FloatProcessor smallImage, FloatProcessor fp, int shrinkFactor) {
		...
		for (int x = 0; x < width; x++) // x-interpolation of the first smallImage line
			line1[x] = sPixels[xSmallIndices[x]] * xWeights[x] + sPixels[xSmallIndices[x] + 1] * (1f - xWeights[x]);
	...
	void makeInterpolationArrays(int[] smallIndices, float[] weights, int length, int smallLength, int shrinkFactor) {
		for (int i = 0; i < length; i++) {
			...
			float distance = (i + 0.5f) / shrinkFactor - (smallIndex + 0.5f); // distance of pixel centers (in
																				// smallImage pixels)
	...
	/**
	 * Replace the pixels by the mean or maximum in a 3x3 neighborhood. No snapshot
	 * is required (less memory needed than e.g., fp.smooth()). When used as maximum
	 * filter, it returns the average change of the pixel value by this operation
	 */
...
/**
 * A rolling ball (or actually a square part thereof) Here it is also determined
 * whether to shrink the image
 */
class RollingBall {
	 ...
	 * center of the sphere containing it. The patch is located in the top half of
	 * this sphere. The vertical axis of the sphere passes through the center of the
	 * patch. The projection of the patch in the xy-plane below is a square.
	 */
	void buildRollingBall(double ballradius, int arcTrimPer) {
		...
		for (int y = 0, p = 0; y < width; y++)
			for (int x = 0; x < width; x++, p++) {
				...
				// -Float.MAX_VALUE might be better than 0f, but gives different results than
				// earlier versions
			}
	}
