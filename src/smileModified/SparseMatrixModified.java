...
public class SparseMatrixModified extends IMatrixModified implements Iterable<SparseMatrixModified.Entry> {
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SparseMatrixModified.class);
    ...
    public class Entry {
        ...
        public String toString() {
            return String.format("(%d, %d):%s", i, j, StringsModified.format(x));
        }
    }
    ...
    public SparseMatrixModified(double[][] A) {
        this(A, 100 * MathExModified.EPSILON);
    }
    ...
    public SparseMatrixModified clone() {
        return new SparseMatrixModified(m, n, nonzeros.clone(), rowIndex.clone(), colIndex.clone());
    }
    ...
    public void mv(TransposeModified trans, double alpha, double[] x, double beta, double[] y) {
        int k = trans == TransposeModified.NO_TRANSPOSE ? m : n;
        double[] ax = y;
        if (beta == 0.0) {
            Arrays.fill(y, 0.0);
        } else {
    ...
    public SparseMatrixModified transpose() {
        SparseMatrixModified trans = new SparseMatrixModified(n, m, nonzeros.length);
    ...
    public SparseMatrixModified mm(SparseMatrixModified B) {
        ...
        SparseMatrixModified C = new SparseMatrixModified(m, n, nzmax);
        int[] Cp = C.colIndex;
        int[] Ci = C.rowIndex;
        ...
        for (int j = 0; j < n; j++) {
            if (nz + m > nzmax) {
                ...
                Ci = Ci2;
                Cx = Cx2;
                C = new SparseMatrixModified(m, n, Cx2, Ci2, Cp);
            }
    ...
    public SparseMatrixModified ata() {
        SparseMatrixModified AT = transpose();
        return AT.aat(this);
    }
    ...
    private SparseMatrixModified aat(SparseMatrixModified AT) {
        ...
        SparseMatrixModified aat = new SparseMatrixModified(m, m, nvals);
    ...
    public static SparseMatrixModified harwell(Path path) throws IOException {
        ...
        try (InputStream stream = Files.newInputStream(path);
            ...
            return new SparseMatrixModified(nrow, ncol, data, rowIndex, colIndex);
        }
    }
