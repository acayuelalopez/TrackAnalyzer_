...
public abstract class IMatrixModified implements Cloneable, Serializable {
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(IMatrixModified.class);
    ...
    private String str(int i, int j) {
        return smileModified.StringsModified.format(get(i, j), true);
    }
     ...
     */
    public abstract void mv(TransposeModified trans, double alpha, double[] x, double beta, double[] y);
    ...
    public double[] mv(double[] x) {
        ...
        mv(TransposeModified.NO_TRANSPOSE, 1.0, x, 0.0, y);
        return y;
    }
    ...
    public void mv(double alpha, double[] x, double beta, double[] y) {
        mv(TransposeModified.NO_TRANSPOSE, alpha, x, beta, y);
    }
    ...
    public double[] tv(double[] x) {
        ...
        mv(TransposeModified.TRANSPOSE, 1.0, x, 0.0, y);
        return y;
    }
    ...
    public void tv(double alpha, double[] x, double beta, double[] y) {
        mv(TransposeModified.TRANSPOSE, alpha, x, beta, y);
    }
    ...
    public double eigen(double[] v) {
        return eigen(v, 0.0, Math.max(1.0E-6, nrow() * MathExModified.EPSILON), Math.max(20, 2 * nrow()));
    }
        ...
        try (LineNumberReader reader = new LineNumberReader(Files.newBufferedReader(path));
            ...
            if (format.equals("coordinate")) {
                ...
                for (int k = 0; k < nz; k++) {
                    ...
                    SparseArrayModified row = rows.get(i);
                    row.set(j, x);
                    colSize[j] += 1;
                ...
                for (int i = 0; i < nrow; i++) {
                    for (SparseArrayModified.Entry e :rows.get(i)) {
                        int j = e.i;
                ...
                return new SparseMatrixModified(nrow, ncol, x, rowIndex, colIndex);
    ...
    public double solve(double[] b, double[] x) {
        return solve(b, x, Jacobi(), 1E-6, 1, 2 * Math.max(nrow(), ncol()));
    }
