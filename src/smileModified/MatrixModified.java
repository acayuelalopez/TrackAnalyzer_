...
public class MatrixModified extends IMatrixModified {
    ...
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(MatrixModified.class);
    ...
    public MatrixModified clone() {
        ...
        if (layout() == LayoutModified.COL_MAJOR) {
            matrix = new MatrixModified(m, n, ld, A.clone());
        } else {
            matrix = new MatrixModified(m, n);
            for (int j = 0; j < n; j++) {
                for (int i = 0; i < m; i++) {
    ...
    public MatrixModified rows(int... rows) {
        MatrixModified x = new MatrixModified(rows.length, n);
    ...
    public MatrixModified cols(int... cols) {
        MatrixModified x = new MatrixModified(m, cols.length);
    ...
    public MatrixModified transpose(boolean share) {
        MatrixModified matrix;
        if (share) {
            if (layout() == LayoutModified.ROW_MAJOR) {
                matrix = new MatrixModified(n, m, ld, A);
            } else {
                matrix = new RowMajor(n, m, ld, A);
            }
        } else {
            matrix = new MatrixModified(n, m);
            for (int j = 0; j < m; j++) {
                for (int i = 0; i < n; i++) {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof MatrixModified)) {
            return false;
        }

        return equals((MatrixModified) o, 1E-10);
    }
    ...
    public boolean equals(MatrixModified o, double epsilon) {
        ...
        for (int j = 0; j < n; j++) {
            for (int i = 0; i < m; i++) {
                if (!MathExModified.isZero(get(i, j) - o.get(i, j), epsilon)) {
                    return false;
                }
            }
        }
    ...
    public MatrixModified add(double alpha, double[] x, double[] y) {
        ...
        if (isSymmetric() && x == y) {
            BLASModified.engine.syr(layout(), uplo, m, alpha, x, 1, A, ld);
        } else {
            BLASModified.engine.ger(layout(), m, n, alpha, x, 1, y, 1, A, ld);
        }
    ...
    public MatrixModified inverse() {
        ...
        MatrixModified lu = clone();
        MatrixModified inv = eye(n);
        int[] ipiv = new int[n];
        if (isSymmetric()) {
            int info = LAPACKModified.engine.sysv(lu.layout(), uplo,  n, n, lu.A, lu.ld, ipiv, inv.A, inv.ld);
            if (info != 0) {
                throw new ArithmeticException("SYSV fails: " + info);
            }
        } else {
            int info = LAPACKModified.engine.gesv(lu.layout(), n, n, lu.A, lu.ld, ipiv, inv.A, inv.ld);
            if (info != 0) {
        ...
        if (uplo != null) {
            if (diag != null) {
                if (alpha == 1.0 && beta == 0.0 && x == y) {
                    BLASModified.engine.trmv(layout(), uplo, trans, diag, m, A, ld, y, 1);
                } else {
                    BLASModified.engine.gemv(layout(), trans, m, n, alpha, A, ld, x, 1, beta, y, 1);
                }
            } else {
                BLASModified.engine.symv(layout(), uplo, m, alpha, A, ld, x, 1, beta, y, 1);
            }
        } else {
            BLASModified.engine.gemv(layout(), trans, m, n, alpha, A, ld, x, 1, beta, y, 1);
        }
    ...
    public void mv(double[] work, int inputOffset, int outputOffset) {
        ...
        mv(TransposeModified.NO_TRANSPOSE, 1.0, xb, 0.0, yb);
    }
    ...
    public void tv(double[] work, int inputOffset, int outputOffset) {
        ...
        mv(TransposeModified.TRANSPOSE, 1.0, xb, 0.0, yb);
    }
    ...
    public MatrixModified mm(TransposeModified transA, MatrixModified A, TransposeModified transB, MatrixModified B, double alpha, double beta) {
        if (A.isSymmetric() && transB == TransposeModified.NO_TRANSPOSE && B.layout() == layout()) {
            BLASModified.engine.symm(layout(), SideModified.LEFT, A.uplo, m, n, alpha, A.A, A.ld, B.A, B.ld, beta, this.A, ld);
        } else if (B.isSymmetric() && transA == TransposeModified.NO_TRANSPOSE && A.layout() == layout()) {
            BLASModified.engine.symm(layout(), SideModified.RIGHT, B.uplo, m, n, alpha, B.A, B.ld, A.A, A.ld, beta, this.A, ld);
        } else {
            if (layout() != A.layout()) {
                transA = flip(transA);
                A = A.transpose();
            ...
            BLASModified.engine.gemm(layout(), transA, transB, m, n, k, alpha, A.A, A.ld, B.A, B.ld, beta, this.A, ld);
        }
        return this;
    }
    ...
    public MatrixModified ata() {
        MatrixModified C = new MatrixModified(n, n);
    ...
    public MatrixModified aat() {
        MatrixModified C = new MatrixModified(m, m);
    ...
    public MatrixModified mm(MatrixModified B) {
        ...
        MatrixModified C = new MatrixModified(m, B.n);
    ...
    public MatrixModified mt(MatrixModified B) {
        ...
        MatrixModified C = new MatrixModified(m, B.m);
    ...
    public MatrixModified tm(MatrixModified B) {
        ...
        MatrixModified C = new MatrixModified(n, B.n);
    ...
    public MatrixModified tt(MatrixModified B) {
        ...
        MatrixModified C = new MatrixModified(n, B.m);
    ...
    public LU lu(boolean overwrite) {
        MatrixModified lu = overwrite ? this : clone();
    ...
    public QR qr(boolean overwrite) {
        MatrixModified qr = overwrite ? this : clone();
    ...
    public SVD svd(boolean vectors, boolean overwrite) {
        ...
        MatrixModified W = overwrite ? this : clone();
        if (vectors) {
            MatrixModified U = new MatrixModified(m, k);
            MatrixModified VT = new MatrixModified(k, n);

            int info = LAPACKModified.engine.gesdd(W.layout(), SVDJobModified.COMPACT, W.m, W.n, W.A, W.ld, s, U.A, U.ld, VT.A, VT.ld);
            if (info != 0) {
            ...
            MatrixModified U = new MatrixModified(1, 1);
            MatrixModified VT = new MatrixModified(1, 1);

            int info = LAPACKModified.engine.gesdd(W.layout(), SVDJobModified.NO_VECTORS, W.m, W.n, W.A, W.ld, s, U.A, U.ld, VT.A, VT.ld);
            if (info != 0) {
                logger.error("LAPACK GESDD error code: {}", info);
                throw new ArithmeticException("LAPACK GESDD error code: " + info);
            }
    ...
    public EVD eigen(boolean vl, boolean vr, boolean overwrite) {
        ...
        MatrixModified eig = overwrite ? this : clone();
        if (isSymmetric()) {
            double[] w = new double[n];
            int info = LAPACKModified.engine.syevd(eig.layout(), vr ? EVDJobModified.VECTORS : EVDJobModified.NO_VECTORS, eig.uplo, n, eig.A, eig.ld, w);
            if (info != 0) {
            ...
            MatrixModified Vl = vl ? new MatrixModified(n, n) : new MatrixModified(1, 1);
            MatrixModified Vr = vr ? new MatrixModified(n, n) : new MatrixModified(1, 1);
            int info = LAPACKModified.engine.geev(eig.layout(), vl ? EVDJobModified.VECTORS : EVDJobModified.NO_VECTORS, vr ? EVDJobModified.VECTORS : EVDJobModified.NO_VECTORS, n, eig.A, eig.ld, wr, wi, Vl.A, Vl.ld, Vr.A, Vr.ld);
            if (info != 0) {
    ...
    public static class SVD implements Serializable {
        ...
        public MatrixModified diag() {
            MatrixModified S = new MatrixModified(U.m, V.m);
        ...
        private double rcond() {
            return 0.5 * Math.sqrt(m + n + 1) * s[0] * MathExModified.EPSILON;
        }
        ...
        public double condition() {
            ...
            return (s[0] <= 0.0 || s[s.length - 1] <= 0.0) ? Double.POSITIVE_INFINITY : s[0] / s[s.length - 1];
        }
        ...
        public MatrixModified range() {
            ...
            MatrixModified R = new MatrixModified(m, r);
            for (int j = 0; j < r; j++) {
                for (int i = 0; i < m; i++) {
                    R.set(i, j, U.get(i, j));
                }
        ...
        public MatrixModified nullspace() {
            ...
            MatrixModified N = new MatrixModified(n, nr);
            for (int j = 0; j < nr; j++) {
                for (int i = 0; i < n; i++) {
        ...
        public MatrixModified pinv() {
            ...
            return adb(TransposeModified.NO_TRANSPOSE, V, sigma, TransposeModified.TRANSPOSE, U);
        }
    ...
    public static class EVD implements Serializable {
         ...
         */
        public MatrixModified diag() {
            MatrixModified D = MatrixModified.diag(wr);
    ...
    public static class LU implements Serializable {
         ...
         */
        public MatrixModified inverse() {
            MatrixModified inv = MatrixModified.eye(lu.n);
            solve(inv);
            return inv;
        }
        ...
        public void solve(MatrixModified B) {
            ...
            int ret = LAPACKModified.engine.getrs(lu.layout(), TransposeModified.NO_TRANSPOSE, lu.n, B.n, lu.A, lu.ld, ipiv, B.A, B.ld);
            if (ret != 0) {
    ...
    public static class Cholesky implements Serializable {
        ...
        public void solve(MatrixModified B) {
            ...
            int info = LAPACKModified.engine.potrs(lu.layout(), lu.uplo, lu.n, B.n, lu.A, lu.ld, B.A, B.ld);
            if (info != 0) {
    ...
    public static class QR implements Serializable {
        ...
        public MatrixModified R() {
            int n = qr.n;
            MatrixModified R = MatrixModified.diag(tau);
            for (int i = 0; i < n; i++) {
                for (int j = i; j < n; j++) {
        ...
        public MatrixModified Q() {
            ...
            int n = qr.n;
            int k = Math.min(m, n);
            MatrixModified Q = qr.clone();
            int info = LAPACKModified.engine.orgqr(qr.layout(), m, n, k, Q.A, qr.ld, tau);
            if (info != 0) {
        ...
        public void solve(MatrixModified B) {
            ...
            int info = LAPACKModified.engine.ormqr(qr.layout(), SideModified.LEFT, TransposeModified.TRANSPOSE, B.nrow(), B.ncol(), k, qr.A, qr.ld, tau, B.A, B.ld);
            if (info != 0) {
                logger.error("LAPACK ORMQR error code: {}", info);
                throw new IllegalArgumentException("LAPACK ORMQR error code: " + info);
            }

            info = LAPACKModified.engine.trtrs(qr.layout(), UPLOModified.UPPER, TransposeModified.NO_TRANSPOSE, DiagModified.NON_UNIT, qr.n, B.n, qr.A, qr.ld, B.A, B.ld);
