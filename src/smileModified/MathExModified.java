...
public class MathExModified {
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(MathExModified.class);
     ...
     */
    private static final ThreadLocal<RandomModified> random = new ThreadLocal<smileModified.RandomModified>() {
        protected RandomModified initialValue() {
            synchronized(DEFAULT_SEEDS) {
                ...
                if (nextSeed < DEFAULT_SEEDS.length) {
                    return new RandomModified(DEFAULT_SEEDS[nextSeed++]);
                } else {
                    return new RandomModified(generateSeed());
                }
            }
        }
    };
    ...
    public static void reverse(int[] a) {
        ...
        while (i < j) {
            SortModified.swap(a, i++, j--);  // code for swap not shown, but easy enough
        }
    }
    ...
    public static double KullbackLeiblerDivergence(SparseArrayModified p, double[] q) {
        ...
        double kl = 0.0;
        while (iter.hasNext()) {
            SparseArrayModified.Entry b = iter.next();
            int i = b.i;
            if (q[i] > 0) {
                intersection = true;
