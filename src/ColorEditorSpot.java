...
public class ColorEditorSpot extends AbstractCellEditor implements TableCellEditor {
	...
	public ColorEditorSpot(JList<String> featureList) {
		...
		ImageIcon iconEdit = createImageIcon("images/edit.png");
		...
		ImageIcon iconDelete = createImageIcon("images/bin.png");
		...
		ImageIcon iconOk = createImageIcon("images/add.png");
		...
		okButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (tableC.getSelectedRowCount() == 1) {
					...
					classList = FirstWizardPanel.classList;
					modelListClass = FirstWizardPanel.modelListClass;
					...
					if (modelListClass.getSize() == 0) {
						...
						for (int x = 0; x < FirstWizardPanel.modelSpot.getRowCount(); x++)
							for (int u = 0; u < features.size(); u++)
								if (((Double.parseDouble(FirstWizardPanel.tableSpot.getModel()
										.getValueAt(x,
												FirstWizardPanel.tableSpot.getColumn(features.get(u)).getModelIndex())
										.toString()) >= Double.parseDouble(featureMin.get(u)))
										&& (Double.parseDouble(FirstWizardPanel.tableSpot.getModel()
												.getValueAt(x,
														FirstWizardPanel.tableSpot.getColumn(features.get(u))
																.getModelIndex())
												.toString()) <= Double.parseDouble(featureMax.get(u))))) {
									FirstWizardPanel.tableSpot.getModel().setValueAt(labelsTableC,
											FirstWizardPanel.tableSpot.convertRowIndexToModel(x),
											FirstWizardPanel.tableSpot.convertColumnIndexToModel(1));
								}
